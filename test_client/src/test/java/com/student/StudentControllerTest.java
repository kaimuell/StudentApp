...
public class StudentControllerTest {
	...
	@Test
	void testGetAll() {
		ResponseEntity<Collection<Student>> responseEntity = new RestTemplate().exchange(
				URL, HttpMethod.GET, null,
				new ParameterizedTypeReference<Collection<Student>>() {}
		);
		assertThat(responseEntity.getStatusCode(), equalTo(HttpStatus.OK));
		Objects.requireNonNull(responseEntity.getBody()).forEach(
				System.out::println
		);
	}
	@Test
	void testGetOnePathParam() {
		ResponseEntity<Student> responseEntity = new RestTemplate().exchange(
				URL + "/{id}",
				HttpMethod.GET,
				null,
				Student.class,
		...
		**/
		...
		ResponseEntity<String> responseEntity =
				new RestTemplate().exchange(
						URL + "/single?id={id}", HttpMethod.GET, new HttpEntity<String>(headers), String.class,3);
		System.out.println(responseEntity.getBody());
		headers = new HttpHeaders();
